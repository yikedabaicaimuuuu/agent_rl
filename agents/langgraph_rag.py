...
def create_rag_graph(
    ...
    # 注入统一 logger（若外部未注入）
    def _ensure_logger(agent, logger):
        try:
            if hasattr(agent, "logger") and getattr(agent, "logger") is None:
                setattr(agent, "logger", logger)
            elif not hasattr(agent, "logger"):
                setattr(agent, "logger", logger)
        except Exception:
            pass
    ...
    def query_optimizer(state: AgentState) -> AgentState:
        try:
            ...
            # 确保各 agent 注入同一个 logger
            _ensure_logger(reasoning_agent, state["logger"])
            _ensure_logger(retrieval_agent, state["logger"])
            _ensure_logger(generation_agent, state["logger"])
            _ensure_logger(evaluation_agent, state["logger"])

            if state["logger"]:
                state["logger"].add_reason(f"[query_optimizer.start] q={state['question']}")
            ...
            if state["logger"]:
                state["logger"].add_reason(f"[query_optimizer.refined] {refined_query}")
            ...
            if state["logger"]:
                state["logger"].add_reason(f"[query_optimizer.error] {e}")
            return {
                **state,
        ...
        try:
            ...
            def _to_lc_doc(d) -> Document:
                ...
                if isinstance(d, dict):
                    ...
                    txt = str(getattr(d, "page_content", "") or d)  # 兜底
                if len(txt) > 3000:
                    txt = txt[:3000]
            ...
            if state["logger"]:
                state["logger"].add_reason(f"[retriever.error] {e}")
            return {
                **state,
                "docs": [],
    ...
    def generator(state: AgentState) -> AgentState:
        try:
            ...
            if state["logger"]:
                state["logger"].add_reason(f"[generator.error] {e}")
            return {
                **state,
    ...
    def router(state: AgentState) -> AgentState:
        ...
        if state["logger"]:
            state["logger"].set_router_action(action)
        ...
        if action == 'requery' and requery_count < state['max_requeries']:
            return {**state, 'next_step': 'requery', 'requery_count': requery_count+1, 'attempts': attempts+1}
        if action == 'regenerate' and regenerate_count < state['max_regenerates']:
            return {**state, 'next_step': 'regenerate', 'regenerate_count': regenerate_count+1, 'attempts': attempts+1}
    ...
    def requery_optimizer(state: AgentState) -> AgentState:
        try:
            ...
            if state["logger"]:
                state["logger"].add_reason(f"[requery.refined] {refined_query}")
            ...
            if state["logger"]:
                state["logger"].add_reason(f"[requery.error] {e}")
            return {
                **state,
    ...
    def finalizer(state: AgentState) -> AgentState:
        ...
        if state["logger"]:
            if state.get("answer"):
                state["logger"].set_final_answer(state["answer"])
            state["logger"].commit()  # 写入 JSONL

        retrieve_cache.clear()
        return {**state, "metrics": {**state["metrics"], "total_time": total_time}}
...
def run_rag_pipeline(
    ...
    **kwargs
    ...
    logger: Optional[TrajectoryLogger] = kwargs.get("logger") or (TrajectoryLogger(out_dir="runs/trajectories") if TrajectoryLogger else None)
    if logger:
    ...
    if not use_router or graph is None:
        ...
        try:
            ...
            docs = []
            metrics.update({"retrieval_time": 0.0, "doc_count": 0, "context_precision": 0.0, "context_recall": 0.0})
        ...
        try:
            ...
            print(f"[pipeline.error] generation: {type(e).__name__}: {e}")  # 新增：直接打印异常
            import traceback; traceback.print_exc()  # 必加：直接显示哪一行还在裸用
            answer, faith, rel, noise, semf1 = "Sorry, I encountered an issue while generating an answer.", 0.0, 0.0, 1.0, 0.0
